...
def score_all_from_cache(supabase=None) -> dict[str, float]:
    ...
    try:
        ...
        raise RuntimeError(f"Could not read cached_scores for dimension 'green': {exc}") from exc

    rows = response.data
    if not rows:
        raise RuntimeError("cached_scores returned 0 rows for dimension 'green' — cache may not be populated")

    result = {}
    for row in rows:
        if row.get("needs_retry"):
            logging.warning("[green] District %s has needs_retry=True — score is a placeholder", row["district"])
