...
def score_all_from_cache(supabase=None) -> dict[str, float]:
    ...
    try:
        ...
        raise RuntimeError(f"Could not read cached_scores for dimension 'crime': {exc}") from exc

    rows = response.data
    if not rows:
        raise RuntimeError("cached_scores returned 0 rows for dimension 'crime' — cache may not be populated")

    result = {}
    for row in rows:
        if row.get("needs_retry"):
            logging.warning("[crime] District %s has needs_retry=True — score is a placeholder", row["district"])
